...
it("runs an AI SDK model instance with external tools and restores its history from R2", async () => {
  const api = client();
  ...
  async function settle(status: string) {
    await expect
      .poll(async () => {
        await runDurableObjectAlarm(stub(session.id));
        await runDurableObjectAlarm(env.AI_HARNESS.getByName(session.id));
        await runDurableObjectAlarm(stub(session.id));
        return (await api.beta.agents.sessions.retrieve(session.id)).status;
      })
      .toBe(status);
  }
  await settle("requires_action");
  const pending = (await api.beta.agents.sessions.retrieve(session.id)).required_actions[0];
  if (pending?.type !== "function_call") throw new Error("Expected a tool call");
  await api.beta.agents.sessions.events.create(session.id, {
    events: [
      {
        type: "agent.session.input.tool_result",
  ...
  await api.beta.agents.sessions.events.create(session.id, {
    events: [
      {
        type: "agent.session.input.message",
        input: [{ role: "user", content: [{ type: "input_text", text: "Recall the value" }] }],
      },
    ],
  });
  await settle("idle");
  const restored = (await api.beta.agents.sessions.items.list(session.id)).data;
  expect(
    restored.filter((item) => item.type === "message" && item.role === "assistant"),
  ).toHaveLength(2);
  expect(JSON.stringify(restored)).toContain("history restored");
});
