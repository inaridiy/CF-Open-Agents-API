...
type PromptMessage = StreamOptions["prompt"][number];
type ToolDefinition = Extract<NonNullable<StreamOptions["tools"]>[number], { type: "function" }>;
type Step = { name: string | undefined; input: object };

interface ScriptContext {
  history: string;
  request: string;
  restored: boolean;
  toolResults: PromptMessage[];
  definitions: ToolDefinition[];
  codex: ToolDefinition | undefined;
}

const findTool = (name: string, definitions: ToolDefinition[]) =>
  definitions.find((tool) => tool.name === name || tool.name.endsWith(`__${name}`));

function scriptContext({ prompt, tools }: StreamOptions): ScriptContext {
  ...
  return { history, request, restored, toolResults, definitions, codex };
}

function reply(step: Step | undefined, answer: string): StreamResult {
...
function once(step: Step | undefined, answer: string, toolResults: PromptMessage[]): StreamResult {
  return reply(toolResults.length ? undefined : step, answer);
}

function shell(command: string, codex: ToolDefinition | undefined, definitions: ToolDefinition[]) {
  if (codex)
    return {
      name: codex.name,
  ...
  return {
    name: definitions.find((tool) => tool.name === "bash" || tool.name.endsWith("__bash"))?.name,
    input: { command, description: "Delegation proof" },
  };
}

function programmaticProof(context: ScriptContext): StreamResult {
  const { request, toolResults, definitions } = context;
...
function delegationProof(context: ScriptContext): StreamResult {
  const { toolResults, definitions } = context;
...
function buildSteps(
  ...
  if (restored)
    return [
      {
        name: findTool("bash", definitions)?.name,
        input: {
          command:
  ...
  return [
    { name: findTool("write", definitions)?.name, input: { ...file, content: "before-edit" } },
    { name: findTool("edit", definitions)?.name, input: { ...file, ...edit } },
    { name: findTool("read", definitions)?.name, input: { ...file } },
    {
      name: findTool("bash", definitions)?.name,
      input: {
        command:
...
function nativeStep(
  ...
  toolResults: PromptMessage[],
  mcpProof: boolean,
  mcp: ToolDefinition | undefined,
  search: ToolDefinition | undefined,
): Step | undefined {
  if (!JSON.stringify(toolResults).includes("sandbox-only")) return steps[0];
  if (!mcpProof)
    return mcp
      ? { name: mcp.name, input: {} }
...
function externalStep(
  ...
  const direct = steps[toolResults.length];
  if (direct !== undefined) return direct;
  if (!restored && !mcpProof)
    return mcp ? { name: mcp.name, input: {} } : { name: undefined, input: {} };
  return undefined;
}

function workspaceProof(context: ScriptContext): StreamResult {
  const { restored, toolResults, definitions, codex } = context;
  const claude = definitions.some((tool) => tool.name.startsWith("mcp__"));
  const file: Record<string, string> = claude
    ? { file_path: "/workspace/proof.txt" }
    : { filePath: "/workspace/proof.txt" };
  const edit: Record<string, string> = claude
    ? { old_string: "before-edit", new_string: "sandbox-only" }
    : { oldString: "before-edit", newString: "sandbox-only" };
  const steps = buildSteps(codex, restored, file, edit, definitions);
  ...
  const step = codex
    ? nativeStep(steps, toolResults, mcpProof, mcp, search)
    : externalStep(steps, toolResults, restored, mcpProof, mcp);
...
async function scripted(options: StreamOptions): Promise<StreamResult> {
  const context = scriptContext(options);
  const { history, request, restored, toolResults, definitions, codex } = context;
  ...
  if (request.includes("child-proof"))
    return once(
      shell(
        "printf CHILD_FILE > /workspace/child.txt && cat /workspace/child.txt",
        codex,
        definitions,
      ),
      "CHILD_DONE",
  ...
  if (request.includes("verify-ported")) {
    ...
    return once(
      shell("cat /workspace/proof.txt", codex, definitions),
      "Ported history restored.",
      toolResults,
    );
  }
  if (request.includes("verify-child-file")) {
    ...
    return once(
      shell("cat /workspace/child.txt", codex, definitions),
      "Child file present.",
      toolResults,
    );
  }
...
const mcpResult = (method: string | undefined) => {
  ...
  if (method === "tools/call") return { content: [{ type: "text", text: "MCP_LOOKUP_PROOF" }] };
  return {};
};
...
export class McpFixture extends WorkerEntrypoint {
  override async fetch(request: Request): Promise<Response> {
    ...
    return Response.json({ jsonrpc: "2.0", id: body.id, result: mcpResult(body.method) });
  }
}
