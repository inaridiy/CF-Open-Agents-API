...
/**
 * Local development variables for the two example Workers: one random API token in
 * both files. Existing files are kept unless `--force`.
 * @param {string} root
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (results.some((result) => result.status === "skipped"))
    console.log("Existing files were kept; pass --force to regenerate both with a new token.");
  console.log(
