...
/** A chunk that carries output rather than the finish. */
type OutputChunk = Exclude<ModelChunk, { type: "finish" }>;
...
export async function encodeModelResponse(
  ...
  const anthropicItem = (item: Item) => {
    ...
    if (item.type === "reasoning") return { type: "thinking", thinking: item.text, signature: "" };
    return { type: "text", text: item.text };
  };
  const anthropicDelta = (chunk: OutputChunk) => {
    if (chunk.type === "call")
      return { type: "input_json_delta", partial_json: JSON.stringify(chunk.input) };
    if (chunk.type === "reasoning") return { type: "thinking_delta", thinking: chunk.text };
  ...
  /** The first frame of a stream, before any output item exists. */
  function* prelude(): Generator<string> {
  ...
  /** A fresh output item was opened by this chunk. */
  function* itemAdded(index: number, item: Item, chunk: OutputChunk): Generator<string> {
    if (input.protocol === "responses") {
  ...
  /** The chunk's increment on an open item. */
  function* itemDelta(index: number, item: Item, chunk: OutputChunk): Generator<string> {
    if (input.protocol === "responses") {
      if (chunk.type === "text")
      ...
      yield event("content_block_delta", { index, delta: anthropicDelta(chunk) });
      ...
      yield chatChunk(
        chunk.type === "reasoning" ? { reasoning_content: chunk.text } : { content: chunk.text },
      );
  }
  /** Fold one output chunk into its item, streaming the frames it produces. */
  function* accept(chunk: OutputChunk): Generator<string> {
    ...
    if (input.stream && fresh) yield* itemAdded(index, item, chunk);
    if (chunk.type !== "call") item.text += chunk.text;
    if (!input.stream) return;
    yield* itemDelta(index, item, chunk);
  }
  const recordUsage = (chunk: Extract<ModelChunk, { type: "finish" }>) => {
    finish = chunk.reason;
    usage = {
  ...
  /** The complete non-streaming document. */
  const finalDocument = (hasCalls: boolean): string => {
    if (input.protocol === "responses") return JSON.stringify(response("completed"));
  ...
  /** Close every item, then the response. */
  function* itemDone(index: number, item: Item): Generator<string> {
    if (input.protocol === "responses") {
      if (item.type === "text") {
  ...
  function* epilogue(hasCalls: boolean): Generator<string> {
    for (const [index, item] of items.entries()) yield* itemDone(index, item);
    if (input.protocol === "responses")
  ...
  /** The failure frame a stream ends with; a document fails as a whole instead. */
  function* failed(): Generator<string> {
    if (input.protocol === "responses")
  ...
  async function* encode(): AsyncGenerator<string> {
    try {
      ...
      for await (const chunk of source) {
        ...
        if (chunk.type === "finish") recordUsage(chunk);
        else yield* accept(chunk);
      }
