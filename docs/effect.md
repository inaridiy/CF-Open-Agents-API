...
is validated with Effect Schema before writes and reconciliation. The persisted
JSON layout and checkpoint version remain unchanged, so this rewrite requires no
SQL migration or history conversion.
...
and return typed failures directly. `batchSchema`, `executionSchema`,
