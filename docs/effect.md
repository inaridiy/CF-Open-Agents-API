...
| `record-store.ts`   | `RecordStore` (get, require, put, remove, clear, list, append, events, lastEvent, outputKey, purge) and `Transactional` (`transactionSync`). Synchronous and free of Effect. |
...
| `repo.ts`           | `Sync<A>`, `Repo<Tx, E>` and `makeRepo`, the Effect edge of the seam.                                                                                                        |
