...
This is an independent implementation. It aims to let the official client work unchanged, with your own runtime, model and storage. It does not aim to reproduce OpenAI's hosted infrastructure, billing or policy. `GET /cf/v1/capabilities` returns the profile, the deployed presets (`harness`, `model`, `delegates`, `tiers`, `webSearch`) and each harness driver's capability flags.
...
- Files written with Claude Code's `Write` and `Edit` tools produce no `command_execution` item (only shell commands do); the `openai@7.15.0` `AgentSessionItem` union has no file-change item, so the write is visible in the final message and in the artifacts. See [known issues](known-issues.md).
...
| Native subagents         | Native threads (`multi_agent`, `max_concurrent_threads_per_session`)   | `Task` tool with one general-purpose definition, concurrency enforced in `canUseTool`; the parent may pick `haiku`, `sonnet` or `opus` for a subagent, each resolving to the gateway name in the preset's `tiers`, else the preset's model; `tiers.haiku` is also the CLI's small-fast model for its internal helper calls, so it must accept the same protocol | `task` tool with a subagent-mode agent; concurrency is best effort                           |
