...
4. The reconciler polls the supervisor and appends events to the session's event log. SSE listeners read from that log, so a dropped stream loses nothing and never cancels the turn.
