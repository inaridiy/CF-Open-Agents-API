...
const pose = (key: CamKey) => ({
  rotX: key.rotX ?? 0,
  rotY: key.rotY ?? 0,
  rotZ: key.rotZ ?? 0,
  persp: key.persp ?? 1400,
});
...
export const PageCam: React.FC<{
  ...
  const has3D = keys.some(
    (k) =>
      k.rotX !== undefined || k.rotY !== undefined || k.rotZ !== undefined || k.persp !== undefined,
  );
  ...
  const fromPose = pose(a);
  const toPose = pose(b);
  const rotX = lerp(fromPose.rotX, toPose.rotX, t);
  const rotY = lerp(fromPose.rotY, toPose.rotY, t);
  const rotZ = lerp(fromPose.rotZ, toPose.rotZ, t);
  const persp = lerp(fromPose.persp, toPose.persp, t);
